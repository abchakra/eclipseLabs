 ...
 *     Abhishek Chakraborty (JointLogic) - show first,secondlast and last Breadcrumb item when shrinking
 *******************************************************************************/
...
public abstract class BreadcrumbViewer extends StructuredViewer {
		...
		try {
			preservingSelection(new Runnable() {
				...
				public void run() {
					...
					// Disable arrow for last and first
					if (fBreadcrumbItems.size() == 1) {
						BreadcrumbItem firstItem = (BreadcrumbItem) fBreadcrumbItems
								.get(0);
						if (firstItem != null) {
							firstItem.setExpandBlockVisible(false);
						}
					}

					BreadcrumbItem last = (BreadcrumbItem) fBreadcrumbItems
							.get(fBreadcrumbItems.size() - 1);
					if (last != null) {

						if (!showLastItemArrow(last.getData())) {
							last.setExpandBlockVisible(false);
						}
					}
	...
	public abstract boolean showLastItemArrow(Object object);
	...
	private boolean updateOrCreateItem(final int index, final TreePath path,
		...
		if (this.fBreadcrumbItems.size() > index) {
			...
			if (item.getData() != null) {
				item.setExpandBlockVisible(true);
				unmapElement(item.getData());
			}
		} else {
	...
	private boolean updateSize() {
		...
		if (currentWidth > width) {
			...
			while (currentWidth > width
					&& index < this.fBreadcrumbItems.size() - 2) {
				final BreadcrumbItem item = (BreadcrumbItem) this.fBreadcrumbItems
						.get(index);
				if (item.isShowText()) {
			...
			while (currentWidth < width && index >= 1) {
				...
				final BreadcrumbItem bItem = (BreadcrumbItem) this.fBreadcrumbItems
						.get(index);
				if (!bItem.isShowText()) {
					bItem.setShowText(true);
					currentWidth = getCurrentWidth();
					if (currentWidth > width) {
						bItem.setShowText(false);
						index = 0;
					} else {
						requiresLayout = true;
					}
